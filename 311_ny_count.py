...
for name,group in second.groupby('Agency'):#iterrows():#.itertuples():#(index=False):#.sort_values(['Agency','Incident Zip']):#.groupby('Agency'):
	...
	plt.plot(group['Incident Zip'],group['Created Date'], marker='o',linestyle='',label=name,color=colors[count])
	enum+=1
axes = plt.gca()
...
axes.set_ylabel('number of occurrences in year '+year)
plt.legend(numpoints=1, loc='upper right', fontsize='small')#lower right for scatter
#plt.savefig('../output/scatter_ny_'+year+'.png')
plt.show()
plt.close()
