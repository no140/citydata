...
def graph():
	...
	columns = dict()
	columns['Date']=[datetime.datetime.combine(d, datetime.datetime.min.time()) for d in unstacked.index.tolist()]
	...
	scatter = []
	for k,v in columns.items():#range(len(agencies)):
		if verbose>1: print(k)#agencies[i])
		if k != 'Date':
			scatter.append(Scatter(xyvalues, x='Date', y=k, color=agencydict[k]))
	'''
