...
grouped = df[['created','description','hood']].groupby(['hood','description'], as_index=False).agg({'created': "count"})
print(grouped.head(30))

#print(len(grouped[grouped.agency =="NYPD"]))
#print(len(grouped[grouped.agency =="DSNY"]))
complgrp = df[['created','description']].groupby(['description'], as_index=False).count()
print(complgrp)
...
df0 = grouped.merge(complgrp[['description','created']], on='description')
df0.rename(columns={'created_x':'created'}, inplace=True)
df0.rename(columns={'created_y':'complttl'}, inplace=True)
#print(df0)
...
print(df1)
df2 = df1.assign(ratio = lambda x: ((x.created) / (ttlcalls)))
#print(df2.nlargest(3, columns='ratio'))
df3 = df2[['hood','description','ratio']]#
#df4 = df3['ratio'].groupby(level=0, group_keys=True)
print(df3.sort(['hood','ratio'],ascending=False).groupby('hood').head(3))
#print(df3.sort_values(by='ratio', axis=0, ascending=False, inplace=True).groupby('hood').head(3))
