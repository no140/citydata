...
grouped = df[['created','agency','complaint','hood','lat','long']].groupby(['hood','agency','complaint'], as_index=False).agg({'lat': 'mean', 'long':'mean','created': 'count'})#{'created': {'created': "count"}, 'latitude':{'lat': 'mean'}})#,'longitude':{'long', 'mean'}})
...
print('this is population group:')
print(popgrp)
...
df3 = df2[['hood','agency','complaint','ratio','created','lat','long']]#
print(df3)
...
X = df4.as_matrix(columns=df4.columns[2:])
#print(X)
rmax = max(X[:,1])
...
import pygeoj
nf = pygeoj.load("nynta_copy.geojson")
for i,feature in enumerate(nf):
	...
	for row in xf:
		...
		if feature.properties['NTAName'] == row[0] and notFound:
			...
			feature.properties[key4] = row[2]	#complaint
			...
			#del nf[i]
nf.save('nynta_var.geojson')
os.system('echo "var myGeo = $(cat nynta_var.geojson)" > nynta_var.geojson')
#plt.figure(num=None)
#plt.plot(df1['hood'], df1['hoodttl'], 'o')
print(popgrp)#df1 with hoodttl
ax = popgrp[['hood','ratio']].plot(kind='line', title ="ratio of 311 calls per neighborhood population",figsize=(15,10),legend=False, fontsize=12)
ax.set_xlabel("neighborhood",fontsize=12)
ax.set_ylabel("ratio of calls per population",fontsize=12)
plt.show()
'''
