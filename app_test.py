...
agdf['created'] = pd.to_datetime(agdf.created, infer_datetime_format=True, errors='coerce')
agdf['created'] = agdf.created.dt.date
agdf = agdf[['created','agency']].groupby(['created'], as_index=False).count()
#print(agdf)
...
title = city +' latest 311 calls to '+agency
...
for index, row in df.iterrows():
	#print(row['agency'])
	#print(agencydict[row[2]])
	print('.', end="")
	p.scatter(row['long'],row['lat'],fill_color=agencydict[row['agency']], line_width=0.0, legend="bottom_right")#label=row['agency']
'''
show(p)
#p.show()
'''
